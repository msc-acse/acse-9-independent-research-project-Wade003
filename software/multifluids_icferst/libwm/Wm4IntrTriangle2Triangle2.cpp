    ...
    Vector2<Real> akV[24])
{
    ...
    if (iPositive > 0)
    {
        if (iNegative > 0)
        {
            ...
            Vector2<Real> akCV[24];
            int iCQuantity = 0, iCur, iPrv;
            Real fT;

            for (iCur = 0; iCur < riQuantity; iCur++)
            {
              iPrv = imod(iCur - 1, riQuantity);
              assert(iPrv >= 0);
              if (afTest[iPrv] >= (Real)0.0)
                akCV[iCQuantity++] = akV[iPrv];

              // We need to intersect and append the output if this edge crosses the intersecting line
              if ((afTest[iCur] * afTest[iPrv]) < (Real)0.0)
              {
